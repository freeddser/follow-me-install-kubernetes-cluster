...
+ 执行该 docker run 命令的机器 IP 为 192.168.1.13；

## 向 registry push image

将签署 registry 证书的 CA 证书拷贝到 `/etc/docker/certs.d/192.168.1.13:8000` 目录下

``` bash
$ sudo mkdir -p /etc/docker/certs.d/192.168.1.13:8000
$ sudo cp /etc/kubernetes/ssl/ca.pem /etc/docker/certs.d/192.168.1.13:8000/ca.crt
$
```
...
``` bash
$ docker login 192.168.1.13:8000
Username: foo
Password:
Login Succeeded
```
...
``` bash
$ docker tag docker.io/kubernetes/pause 192.168.1.13:8000/zhangjun3/pause
$ docker images |grep pause
docker.io/kubernetes/pause                            latest              f9d5de079539        2 years ago         239.8 kB
192.168.1.13:8000/zhangjun3/pause                        latest              f9d5de079539        2 years ago         239.8 kB
```

将 image push 到私有 registry

``` bash
$ docker push 192.168.1.13:8000/zhangjun3/pause
The push refers to a repository [192.168.1.13:8000/zhangjun3/pause]
5f70bf18a086: Pushed
e16a89738269: Pushed
...
``` bash
$ curl  --user zhangjun3:xxx --cacert /etc/docker/certs.d/192.168.1.13\:8000/ca.crt https://192.168.1.13:8000/v2/_catalog
...
``` bash
$ curl  --user zhangjun3:xxx --cacert /etc/docker/certs.d/192.168.1.13\:8000/ca.crt https://192.168.1.13:8000/v2/zhangjun3/busybox/tags/list
...
``` bash
$ curl -v -H "Accept: application/vnd.docker.distribution.manifest.v2+json" --user zhangjun3:xxx --cacert /etc/docker/certs.d/192.168.1.13\:8000/ca.crt https://192.168.1.13:8000/v2/zhangjun3/busybox/manifests/latest
...
``` bash
$ curl -X DELETE  --user zhangjun3:xxx --cacert /etc/docker/certs.d/192.168.1.13\:8000/ca.crt https://192.168.1.13:8000/v2/zhangjun3/busybox/manifests/sha256:68effe31a4ae8312e47f54bec52d1fc925908009ce7e6f734e1b54a4169081c5
$
```
...
``` bash
$ curl -X DELETE  --user zhangjun3:xxx --cacert /etc/docker/certs.d/192.168.1.13\:8000/ca.crt https://192.168.1.13:8000/v2/zhangjun3/busybox/blobs/sha256:a3ed95caeb02ffe68cdd9fd84406680ae93d633cb16422d00e8a7c22955b46d4
$ curl -X DELETE  --cacert /etc/docker/certs.d/192.168.1.13\:8000/ca.crt https://192.168.1.13:8000/v2/zhangjun3/busybox/blobs/sha256:04176c8b224aa0eb9942af765f66dae866f436e75acef028fe44b8a98e045515
$
```